...
class StageRuntime:
    ...
    def initialize(self, model, inputs_module_destinations,
        ...
        if module_to_stage_map is None:
            ...
            self.modules_with_dependencies = ModulesWithDependencies(
                [model[module] for module in modules])
            self.modules_with_dependencies_diff = ModulesWithDependencies(
                [model[module] for module in modules])
            self.is_criterion = self.stage == (self.num_stages - 1)
            if stage_to_depth_map is not None:
                self.num_warmup_minibatches = stage_to_depth_map[
            ...
            #     EVENT=self.EVENT,
            #     EVENT1=self.EVENT1,
            #     ddp_ranks=self.stage_to_rank[self.stage])
        ...
        for i in range(len(modules)):
            ...
            if self.fp16:
                import apex.fp16_utils as fp16_utils
                modules[i] = fp16_utils.BN_convert_float(modules[i].half())
        modules_diff = self.modules_with_dependencies_diff.modules()
        for i in range(len(modules)):
            # print(type(modules[i]))
            modules_diff[i] = modules_diff[i].cuda()
            if self.fp16:
                import apex.fp16_utils as fp16_utils
                modules_diff[i] = fp16_utils.BN_convert_float(modules_diff[i].half())
        # Initialize all groups in the same order on every worker.
        if stage_to_rank_map is not None:
            groups = []
            for stage in range(self.num_stages):
            ...
            group = None
        master_port = 12347
        self.comm_handler = communication_tyf.CommunicationHandler(
            master_addr=master_addr,
            master_port=master_port,
            rank=self.rank,
            local_rank=self.local_rank,
    ...
    def state_dict_diff(self):
        state_dict = collections.OrderedDict()
        for i, module in enumerate(self.modules_with_dependencies_diff.modules()):
            state_dict["module%d" % i] = module.state_dict()
        if self.fp16:
            state_dict["master_parameters"] = self.master_parameters
        return state_dict
    ...
    def receive_tensors_forward(self):
        ...
        if self.loader_iter is not None:
            ...
            if self.model_type == TRANSLATION:
                ...
                self.tensors_recompute[-1]["input0"]=self.tensors[-1]["input0"].clone()
                self.tensors_recompute[-1]["target"]=self.tensors[-1]["target"].clone()
            ...
            for k in self.tensors[-1].keys():
                self.tensors_recompute[-1][k]=self.tensors[-1][k].clone()
    ...
    def run_forward_recompute(self, recompute_step=False):
        """Run forward pass
        """
        # Receive tensors from previous worker
        if not recompute_step:
            ...
            self.tensors.pop(-1)
            self.tensors.append(self.tensors_recompute[-1])
            tensors = self.tensors[-1]
            # Run forward pass.
            begin_time = time.time()
            self._run_forward(tensors)
    ...
    def _run_forward(self, tensors):
        ...
        # total_params = sum(p.numel() for p in modules[-1].parameters())
        # print("模型的参数数量:", total_params)
        #
        # # 计算模型占用的内存大小
        # model_size = sum(p.element_size() * p.nelement() for p in modules[-1].parameters())
        # print("模型占用的内存大小:", model_size, "bytes")
    ...
    def run_backward(self,if_in_initial=False):
