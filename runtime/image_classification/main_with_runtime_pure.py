...
def main():
    ...
    # model=module.model_vgg16(criterion,[3,35], [0,0,0,0])
    ...
    r = runtime_pure.StageRuntime(
        ...
        model_type=runtime_pure.IMAGE_CLASSIFICATION,
        event=EVENT,
        event1=EVENT1,
    ...
    # optimizer = optim.SGD(r.modules_with_dependencies.modules()[0].parameters(), lr=0.0001, momentum=0.9)
    if args.resume:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # if epoch!=0:
        #     model_vgg = module.model_vgg16(criterion, [5,12,9,12], [0, 0,0,0])
        #     training_tensor_shapes1 = {"input0": input_size, "target": [args.batch_size]}
        #     dtypes1 = {"input0": torch.float32, "target": torch.int64}
        #     for module_id, (stage, inputs, outputs) in enumerate(model_vgg[:-1]):  # Skip last layer (loss).
        #         input_tensors = []
        ...
        #               args.master_addr, args.rank, args.local_rank, args.num_ranks_in_server,training_tensor_shapes1,dtypes1)
...
def train(train_loader, r, optimizer, epoch,inputs_module_destinations, configuration_maps,
    ...
    # for k in r.state_dict().keys():
    #     for k_ in r.state_dict()[k].keys():
    #         print(type(r.state_dict()[k][k_]))
    ...
    #         for k in r.state_dict().keys():
    #             for k_ in r.state_dict()[k].keys():
    #                 r.state_dict_diff()[k][k_][:]=r.state_dict()[k][k_]
            # print(time.time() - time_)
            # for k in r.state_dict().keys():
            #     for k_ in r.state_dict()[k].keys():
            #         print(r.state_dict_diff()[k][k_].data==r.state_dict()[k][k_].data)
        # with torch.no_grad():

        # r.run_forward()
        # time_list.append(r.forward_begin)
        # time_list.append(r.forward_end)
        r.run_forward()
        time_list.append(r.forward_begin)
        time_list.append(r.forward_end)
        # if r.stage == 0:
        #     with torch.no_grad():
        #         r.run_forward_recompute()
        #     r.run_forward_recompute(recompute_step=True)
        # else:
        #     r.run_forward()
        # r.run_forward()
        ...
        # r.run_backward()
        # time_list.append(r.backward_begin)
        # time_list.append(r.backward_end)
        # optimizer.load_new_params()
        # optimizer.step()
        r.run_backward()
        time_list.append(r.backward_begin)
        time_list.append(r.backward_end)
        r.tensors_recompute.clear()
        # r.tensors.clear()
        optimizer.load_new_params()
        optimizer.step()
        if i%100==0:
            ...
            if is_last_stage():
                print(len(time_list))
                save_list_to_txt(time_list,"time_list_1")
            else:
                save_list_to_txt(time_list,"time_list_0")
        # total_bytes = 0
        ...
        # total_gigabytes = total_bytes / (1024 ** 3)  # 转换为GB
        # print(f'Optimizer parameters occupy {total_gigabytes:.2f} GB of GPU memory')
